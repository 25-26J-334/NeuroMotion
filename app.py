...
VOICE_TRANSLATIONS = {
    "ta": {
        "LEFT KNEE VALGUS": "இடது முழங்கால் வளைந்துள்ளது. இடது முழங்காலை உள்ளே வளைக்காதீர்கள்.",
        "RIGHT KNEE VALGUS": "வலது முழங்கால் வளைந்துள்ளது. வலது முழங்காலை உள்ளே வளைக்காதீர்கள்.",
        "EXCESSIVE FORWARD LEAN": "முன்னால் அதிகமாக வளைந்துள்ளீர்கள். நெஞ்சை நிமிர்த்தி உடற்பயிற்சி செய்யவும்.",
        "LEFT KNEE OVER TOES": "இடது முழங்கால் விரல்களைத் தாண்டியுள்ளது. உங்கள் இடுப்பை பின்னால் தள்ளுங்கள்.",
        "RIGHT KNEE OVER TOES": "வலது முழங்கால் விரல்களைத் தாண்டியுள்ளது. உங்கள் இடுப்பை பின்னால் தள்ளுங்கள்.",
        "Danger detected. Check your posture.": "ஆபத்து கண்டறியப்பட்டது. உங்கள் தோரணையை உடனடியாக சரிபார்க்கவும்.",
        "EXCESSIVE BACK ARCH": "முதுகு அதிகமாக வளைந்துள்ளது. உங்கள் வயிற்றை இறுக்கமாக வைக்கவும்.",
        "POOR HEAD POSITION": "தலை நிலை சரியில்லை. தலையை நேராக வைக்கவும்.",
        "LEFT ARM ANGLE INCORRECT": "இடது கை கோணம் சரியில்லை. உங்கள் முழங்கையை உடலோடு ஒட்டி வைக்கவும்.",
        "RIGHT ARM ANGLE INCORRECT": "வலது கை கோணம் சரியில்லை. உங்கள் முழங்கையை உடலோடு ஒட்டி வைக்கவும்.",
        "HIP SAGGING": "இடுப்பு கீழிறங்கியுள்ளது. உங்கள் இடுப்பை சற்று உயர்த்தவும்."
    },
    "si": {
        "LEFT KNEE VALGUS": "වම් දණහිස ඇතුළට නැමී ඇත. වම් දණහිස ඇතුළට නමන්න එපා.",
        "RIGHT KNEE VALGUS": "දකුණු දණහිස ඇතුළට නැමී ඇත. දකුණු දණහිස ඇතුළට නමන්න එපා.",
        "EXCESSIVE FORWARD LEAN": "ඉදිරියට නැමීම වැඩියි. පපුව කෙළින් තබා ගන්න.",
        "LEFT KNEE OVER TOES": "වම් දණහිස පාදයේ ඇඟිලි වලින් ඔබ්බට ගොස් ඇත. ඔබේ උකුළ පසුපසට තල්ලු කරන්න.",
        "RIGHT KNEE OVER TOES": "දකුණු දණහිස පාදයේ ඇඟිලි වලින් ඔබ්බට ගොස් ඇත. ඔබේ උකුළ පසුපසට තල්ලු කරන්න.",
        "Danger detected. Check your posture.": "අනතුරක් හඳුනාගෙන ඇත. ඔබේ ඉරියව්ව වහාම පරීක්ෂා කරන්න.",
        "EXCESSIVE BACK ARCH": "පිටුපස අධික ලෙස නැමී ඇත. ඔබේ බඩ තද කර ගන්න.",
        "POOR HEAD POSITION": "හිසෙහි පිහිටීම නිවැරදි නොවේ. ඔබේ හිස කෙළින් තබා ගන්න.",
        "LEFT ARM ANGLE INCORRECT": "වම් අතෙහි කෝණය නිවැරදි නොවේ. ඔබේ වැලමිට ශරීරයට ආසන්නව තබා ගන්න.",
        "RIGHT ARM ANGLE INCORRECT": "දකුණු අතෙහි කෝණය නිවැරදි නොවේ. ඔබේ වැලමිට ශරීරයට ආසන්නව තබා ගන්න.",
        "HIP SAGGING": "උකුළ පහත් වී ඇත. ඔබේ උකුළ ටිකක් ඔසවන්න."
    }
}

def trigger_voice_alert(text):
    """Trigger voice alert using the VoiceFeedback module"""
    with open('/tmp/voice_debug.log', 'a') as debug_f:
        debug_f.write(f"\\n--- {time.time()} ---\\n")
        debug_f.write(f"trigger_voice_alert called with text: {text}\\n")
        
        enabled = st.session_state.get('voice_alerts_enabled', False)
        debug_f.write(f"voice_alerts_enabled: {enabled}\\n")
        
        if not enabled:
            debug_f.write("returning early: not enabled\\n")
            return
            
        current_time = time.time()
        last_alert_time = st.session_state.get('last_voice_alert_time', 0)
        time_diff = current_time - last_alert_time
        debug_f.write(f"Time diff: {time_diff}\\n")
        
        if time_diff > 4:
            st.session_state.last_voice_alert_time = current_time
            
            lang = st.session_state.get('voice_alert_language', 'en')
            spoken_text = text
            
            if lang in VOICE_TRANSLATIONS:
                if text in VOICE_TRANSLATIONS[lang]:
                    spoken_text = VOICE_TRANSLATIONS[lang][text]
                else:
                    for key, val in VOICE_TRANSLATIONS[lang].items():
                        if key in text:
                            spoken_text = val
                            break
                            
            debug_f.write(f"Speaking: {spoken_text} in lang: {lang}\\n")
            
            if 'voice_feedback' not in st.session_state:
                st.session_state.voice_feedback = VoiceFeedback(enabled=True)
                
            st.session_state.voice_feedback.set_enabled(True)
            st.session_state.voice_feedback.speak(spoken_text, lang)
            debug_f.write("Speak command sent to VoiceFeedback.\\n")
        else:
            debug_f.write("Rate limited.\\n")
...
if 'voice_alert_language' not in st.session_state:
    st.session_state.voice_alert_language = 'en'
if 'page' not in st.session_state:
...
def main_app_jump(db):
    ...
    if st.session_state.voice_alerts_enabled:
        col_lang, col_test = st.columns([3, 1])
        with col_lang:
            lang_options = {"en": "English", "ta": "Tamil", "si": "Sinhala"}
            st.session_state.voice_alert_language = st.selectbox(
                "Voice Language",
                options=["en", "ta", "si"],
                format_func=lambda x: lang_options[x],
                index=["en", "ta", "si"].index(st.session_state.get('voice_alert_language', 'en')),
                key="voice_lang_jump"
            )
        with col_test:
            if st.button("🔊 Test Voice", key="test_voice_jump"):
                trigger_voice_alert("Danger detected. Check your posture.")
    ...
    if st.session_state.voice_alerts_enabled:
        ...
        with col_lang:
            ...
            st.session_state.voice_alert_language = st.selectbox(
                ...
                format_func=lambda x: lang_options[x],
                index=["en", "ta", "si"].index(st.session_state.get('voice_alert_language', 'en')),
                key="voice_lang_squat"
            )
        with col_test:
            if st.button("🔊 Test Voice", key="test_voice_squat"):
                trigger_voice_alert("Testing the voice alert system.")
    ...
    if st.session_state.voice_alerts_enabled:
        ...
        with col_lang:
            ...
            st.session_state.voice_alert_language = st.selectbox(
                ...
                format_func=lambda x: lang_options[x],
                index=["en", "ta", "si"].index(st.session_state.get('voice_alert_language', 'en')),
                key="voice_lang_pushup"
            )
        with col_test:
            if st.button("🔊 Test Voice", key="test_voice_pushup"):
                trigger_voice_alert("Danger detected. Check your posture.")
